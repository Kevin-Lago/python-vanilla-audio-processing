...
def distortion_filter(data, gain, volume):
    ...
    for i in range(int(len(data) / p.get_sample_size(FORMAT))):
        unpacked_data += struct.unpack('f', data[i * p.get_sample_size(FORMAT):i * p.get_sample_size(FORMAT) + p.get_sample_size(FORMAT)])
...
def equalizer(data):
    return distortion_filter(data, 100, .5)
